...
export class OperatorsComponent implements OnInit {
  ...
  loadData() {
    ...
    this.subscription = this.dataService.get(null, null, 3).subscribe(
      (resp) => {
  ...
  openModal(sec, content) {
    ...
    if (sec == 'add') {
      ...
      this.data_form = new FormGroup({
        'user_type': new FormControl(3, Validators.required),
  ...
  onDelete() {
    if (!this.onProgress) {
      ...
      this.dataService.delete(this.selectedData).subscribe(
        (data) => {
          this.dataService.get(null, null, 3).subscribe(
            (resp) => {
  ...
  onSearch(event) {
    this.onProgress = true;
    this.subscription = this.dataService.get(1, event.target.value, 3).subscribe(
      (resp) => {
        this.dataService.data = resp.data;
        this.dataService.dataSubject.next(resp);
        this.onProgress = false;
